...
.. math:: L(\gamma)=\lim \sum_{j=1}\sp N |\gamma(\tau_j)-\gamma(\tau_{j-1})|,
...
.. math:: L(\gamma)=\int_a\sp b |\gamma{{\sp{\prime}}}(t)| \, dt.
...
punktum, en í grennd um þá punkta er :math:`\gamma{{\sp{\prime}}}`
takmarkað.
...
.. math:: \int_Cf \, ds = \int_a\sp b f(\gamma(t)) |\gamma{{\sp{\prime}}}(t)|\, dt.
...
.. math::
   ...
   \begin{aligned}
   \int_c\sp d f(\gamma_1(t)) |\gamma_1{{\sp{\prime}}}(t)|\, dt&=
   \int_c\sp d f(\gamma(h(t))) |\gamma{{\sp{\prime}}}(h(t))h{{\sp{\prime}}}(t)|\, dt\\
   &=
   \int_a\sp bf(\gamma(\tau))|\gamma{{\sp{\prime}}}(\tau)| \, d\tau.\end{aligned}
...
.. math::
   ...
   \int_a^bf(\gamma(t))\gamma{{\sp{\prime}}}(t) \, dt,\\
   \int_C f\, d\bar z =\int_\gamma f\, d\bar z= \int_\gamma
   f\, dx -if\, dy = 
   \int_a^bf(\gamma(t))\overline{\gamma{{\sp{\prime}}}(t)} \, dt.\end{gathered}
...
.. math::
   ...
   \begin{aligned}
   \int_a\sp b f(\gamma_-(t))\alpha_-{{\sp{\prime}}}(t) \, dt &=
   \int_a\sp b f(\gamma(a+b-t))(-\alpha{{\sp{\prime}}}(a+b-t)) \, dt \\
   &=
   -\int_a\sp b f(\gamma(t))\alpha{{\sp{\prime}}}(t) \, dt.\end{aligned}
...
.. math::
   ...
   \gamma{{\sp{\prime}}}(t)= (\beta-\alpha), \qquad t\in [0,1],

 og þar með er

.. math::

   \int_{{{\langle\alpha,\beta\rangle}}} f \, dz = (\beta-\alpha)\int_0\sp 1
...
.. math::
   ...
   \gamma{{\sp{\prime}}}(t)= ire\sp{it}, \qquad t\in [a,b],
...
er fall :math:`F\in {{\cal O}}(X)` þannig að :math:`F{{\sp{\prime}}}=f`
þá er
...
svæði og :math:`f{{\sp{\prime}}}(z)=0` fyrir öll :math:`z\in X`, þá er
:math:`f` fastafall.
...
:math:`\gamma(t)` og horft er í stefnu :math:`\gamma{{\sp{\prime}}}(t)`.
...
Þá segir Green-setningin að um sérhver :math:`f,g\in C\sp 1(X)` gildi
...
2BeRemovedMynd: Stikun á jaðri sem myndaður er úr fjórum vegum
...
Green-setningarinnar. Við tökum :math:`f\in C\sp 1(X)`,
...
vera opið mengi í :math:`{{\mathbb  C}}`, :math:`f\in
{{\cal O}}(X)`, :math:`z\in X` og gerum ráð fyrir að
:math:`\overline S(z,r)\subset X`. Þá gildir

.. math:: f(z)=\dfrac 1{2\pi} \int_0\sp{2\pi}f(z+re\sp{it})\, dt.
...
.. math::
   ...
   \dfrac 1{Q(z)} = \dfrac 1{Q{{\sp{\prime}}}(\alpha_1)(z-\alpha_1)}+\cdots+
   \dfrac 1{Q{{\sp{\prime}}}(\alpha_m)(z-\alpha_m)}.
...
.. math::
   ...
   \dfrac 1{Q{{\sp{\prime}}}(\alpha_1)}\int_{\partial\Omega}\dfrac{f(z)}{z-\alpha_1}\,
   dz 
   +\cdots+
   \dfrac 1{Q{{\sp{\prime}}}(\alpha_m)}\int_{\partial\Omega}\dfrac{f(z)}{z-\alpha_m}\,
   dz.
...
.. math::
   ...
   \dfrac{f(\alpha_j)}{Q{{\sp{\prime}}}(\alpha_j)}.
...
.. math::
   ...
   \int_0\sp{2\pi} R(\cos\theta,\sin \theta) \, d\theta.

   .. _10.2.11:

 Við athugum að ef :math:`z` er á einingarhringnum og við skrifum
:math:`z=e\sp{i\theta}`, þá er
...
.. math:: \int_{-\infty}\sp{+\infty}\dfrac{f(x)}{Q(x)}\, dx,

 þar sem :math:`f` er fágað í grennd um :math:`{{\mathbb  R}}\cup H_+`,
þar sem :math:`H_+=\{z\in
{{\mathbb  C}}; {{\operatorname{Im\, }}}z>0\}` táknar efra hálfplanið og
...
:math:`\gamma_r(t)=re\sp{it}`, :math:`t\in [0,\pi]`. Ef við veljum nú
...
.. math::
   ...
   2\pi i\sum_{\alpha_j\in H_+} \dfrac{f(\alpha_j)}{Q{{\sp{\prime}}}(\alpha_j)}.
...
.. math::
   ...
   \dfrac {f(\gamma_j(t))}{\gamma_j(t)-x-iy}\gamma_j{{\sp{\prime}}}(t)\, dt, 
   \qquad f\in {{\cal O}}(X).
...
.. math::
   ...
   f{{\sp{\prime}}}(z)=\partial_xf(z)=
   \dfrac 1{2\pi i}\sum_{j=1}\sp N \int_{a_j}\sp {b_j}
   \dfrac {f(\gamma_j(t))}{(\gamma_j(t)-x-iy)\sp 2}\gamma_j{{\sp{\prime}}}(t)\, dt


   .. _10.3.1:

 fyrir öll :math:`z\in \Omega`. Á þessari formúlu sjáum við síðan að
:math:`f{{\sp{\prime}}}` er fágað fall og að við megum beita
hlutafleiðunum undir heildið og fáum að afleiðan
:math:`f{{\sp{\prime\prime}}}` af :math:`f{{\sp{\prime}}}` er

.. math::

   f{{\sp{\prime\prime}}}(z)=\partial_x\sp 2f(z)=
   \dfrac 2{2\pi i}\sum_{j=1}\sp N \int_{a_j}\sp {b_j}
   \dfrac {f(\gamma_j(t))}{(\gamma_j(t)-x-iy)\sp 3}\gamma_j{{\sp{\prime}}}(t)\, dt.


   .. _10.3.2:

 Með því að velja :math:`\Omega` sem opnar skífur sem þekja :math:`X`,
þá fáum við að :math:`f\in C\sp{\infty}(X)` og að allar afleiður af
:math:`f` eru fáguð föll. Þegar við fjölluðum um Taylor-raðir í setningu
:ref:‘se:2.3.7‘, þá skilgreindum við hærri
:math:`{{\mathbb  C}}`-afleiður :math:`f\sp{(n)}` af :math:`f` með

.. math:: f\sp{(0)}=f, \qquad f\sp{(n)}=\big(f\sp{(n-1)}){{\sp{\prime}}}, \quad n\geq 1.
...
:math:`|f(z)|\leq M` fyrir öll :math:`z\in \partial
...
:math:`\Omega\cup
\partial \Omega\subset X`. Þá er :math:`f\in {{\cal O}}(X)`.
...
:math:`|f{{\sp{\prime}}}(z)|\leq  M/\varrho`. Við látum
:math:`\varrho\to +\infty` og fáum að :math:`f{{\sp{\prime}}}(z)=0`. Þar
...
:math:`f_n:A\to
{{\mathbb  C}}`. Við segjum að runan :math:`\{f_n\}` stefni á fallið
...
segjum að fallaröðin :math:`\sum_{k=0}\sp\infty f_k` sé *samleitin í
jöfnum mæli* ef runan af hlutsummum :math:`\{\sum_{k=0}\sp n f_k\}` er
samleitin í jöfnum mæli. Ef fallaröðin :math:`\sum_{k=0}\sp\infty |f_k|`
er samleitin í jöfnum mæli á :math:`A`, þá segjum við að
:math:`\sum_{k=0}\sp\infty f_k` sé *alsamleitin í jöfnum mæli
...
2BeRemovedMynd: :math:`f_n\to 0`, *ekki í jöfnum mæli*
...
(*Weierstrass–próf*).   Gerum ráð fyrir að :math:`\sum_{k=0}\sp 
\infty f_k` sé fallaröð á menginu :math:`A`, :math:`\sum_{k=0}\sp
\infty M_k` sé samleitin röð af jákvæðum rauntölum og
:math:``\ 0\|f\_k(a)\| M\_k :math:`` Þá er röðin
:math:`\sum_{k=0}\sp \infty f_k` samleitin í jöfnum mæli á :math:`A`.
...
Látum :math:`A` vera hlutmengi af :math:`{{\mathbb  R}}\sp m` og
...
Látum :math:`A` vera hlutmengi af :math:`{{\mathbb  R}}\sp m` og
:math:`\sum_{k=0}\sp\infty
f_k` vera röð af samfelldum föllum sem er samleitin í jöfnum mæli á
...
bilinu :math:`[a,b]` og að röðin :math:`\sum_{k=0}\sp \infty f_k` sé
...
:math:`A\subset
{{\mathbb  R}}\sp m` í sönnuninni á setninguni hér fyrir framan, fáum
við með sömu röksemdarfærslu:

Setning
^^^^^^^

Látum :math:`A` vera takmarkað hlutmengi í :math:`{{\mathbb  R}}\sp m`
...
Látum :math:`\{f_n\}` vera runu af föllum í :math:`C\sp 1([a,b])`, gerum
ráð fyrir að runan :math:`\{f_n{{\sp{\prime}}}\}` sé samleitin í jöfnum
mæli á :math:`[a,b]` og að til sé :math:`c\in [a,b]` þannig runan
:math:`\{f_n(c)\}` sé samleitin. Þá er stefnir :math:`\{f_n\}` á fall
:math:`f\in C\sp 1([a,b])` í jöfnum mæli á :math:`[a,b]` og

.. math:: f{{\sp{\prime}}}(x)=\lim_{n\to \infty}f_n{{\sp{\prime}}}(x), \qquad x\in [a,b].
...
.. math::
   ...
   \dfrac 1{\zeta-z}=\dfrac 1{\zeta-\alpha}\sum_{n=0}\sp \infty
...
:math:`\zeta\in\partial S(\alpha,\varrho)` og öll :math:`z\in \bar
S(\alpha,\varrho-\varepsilon)`, :math:`\varepsilon<\varrho`, og því
megum við skipta á óendalegu summunni og heildinu í (:ref:‘10.4.1‘). Það
gefur
...
.. math::
   ...
   f(z)=\sum_{n=0}\sp \infty a_n(z-\alpha)\sp n,
...
:math:`{\cal N}(f)=\{\alpha\in X;
f(\alpha)=0\}` kallast *núllstöðvamengi :hover:‘núllstöðvamengi‘* fágaða
...
.. math::
   ...
   f(z)=(z-\alpha)\sp mg(z), \qquad z\in X.
...
að hún sé af stigi :math:`m>0`. Samkvæmt setningu :ref:‘se:10.4.3‘ er
til fall :math:`g\in {{\cal O}}(X)` þannig að
:math:`f(z)=(z-\alpha)\sp mg(z)` fyrir öll :math:`z\in X` og
:math:`g(\alpha)\neq 0`. Fyrst :math:`g` er samfellt, þá er til
:math:`\varepsilon>0`, þannig að :math:`g(z)\neq 0` fyrir öll
:math:`z\in
S(\alpha,\varepsilon)`. Við höfum því :math:`{\cal N}(f)\cap S^\ast
(\alpha,\varepsilon)=\varnothing` og þar með er :math:`{\cal N}(f)`
dreift mengi.
...
:math:`e\sp z=e\sp{x+iy}=e\sp x(\cos y+i\sin y)` sé eina mögulega fágaða
útvíkkunin á veldisvísisfallinu :math:`x\mapsto e\sp x` og að höfuðgrein
...
takmarkað svæði :math:`f\in
{{\cal O}}(X)\cap C(\overline X)` (samfellt á lokuninni
...
2BeRemovedMynd: Einfaldlega og ekki einfaldlega samanhangandi svæði
