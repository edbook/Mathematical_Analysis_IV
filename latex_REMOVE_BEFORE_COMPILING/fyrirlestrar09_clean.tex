...
\subsubsection{Setning} Látum  $P(t,D)=D\sp m+a_{m-1}(t)D\sp{m-1}+\cdots+a_1(t)D+a_0(t)$
...
\subsubsection{Skilgreining}  Runa $\{C_n\}_{n=0}^\infty$,  af
...
\subsubsection{Skilgreining}\tx{Fylkjastaðall :hover:`fylkjastaðall` :hover:`staðall`
 :hover:`staðall!fylkjastaðall`}  Ef $A$ er $\ell\times m$ fylki, $A=(a_{jk})$,
...
\subsubsection{Setning}\tx{Reiknireglur um fylkjastaðal}  (i) Ef $A$ og $B$ eru
...
\subsubsection{Setning}\tx{Samleitnipróf fyrir
fylkjaraðir :hover:`samleitnipróf fyrir
fylkjaraðir` :hover:`samleitni!fylkjaraða`} Látum $\{C_n\}$ vera runu
...
\subsubsection{Skilgreining} Ef $A$ er $m\times m$ fylki með stuðla í $\C$, þá táknum við
...
\subsubsection{Setning}\tx{Cayley--Hamilton} :hover:`Cayley-Hamilton-setningin`
...
\section{Newton-margliður :hover:`margliða!Newton`}
...
\subsubsection{Setning}  Látum $f\in \O(\C)$, $\alpha_1,\dots,\alpha_\ell$ vera ólíka
