...
:math:`C^ m(X)` tákna mengi allra :math:`m` sinnum :hover:‘samfellt
...
:hover:`fágað fall` á opna menginu :math:`X` ef :math:`f\in C^1(X)` og :math:`f` er :math:`{{\mathbb  C}}`–deildanlegt í sérhverjum
...
:math:`a=({\alpha},{\beta})` og sjáum að (:ref:`Link title <4.2.10>`) jafngildir því
að hægt sé að rita
...
fall :math:`\lambda:X\to {{\mathbb  C}}` kallast *logri á :math:`X`* :hover:`logri` ef
